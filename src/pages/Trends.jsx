...
export default function Trends() {
  ...
  const handleMarkHotspotsRead = async () => {
    ...
    try {
      ...
      showToast('Could not mark hotspot alerts as read. Check your connection and try again.', 'error');
    } finally {
  ...
  if (monthKeysForAlerts.length >= 2) {
    const last = byMonthForAlerts[monthKeysForAlerts[monthKeysForAlerts.length - 1]];
    const prev = byMonthForAlerts[monthKeysForAlerts[monthKeysForAlerts.length - 2]];
    if (prev > 0 && last > prev * 1.3) {
      alerts.push({ type: 'danger', icon: Icons.AlertTriangle, msg: `Crime surge: ${last} incidents this month vs ${prev} last month (+${((last / prev - 1) * 100).toFixed(0)}%)` });
    }
  }
  ...
  Object.entries(byTypeForAlerts).forEach(([type, count]) => {
    ...
    if (rate > 25) alerts.push({ type: 'warning', icon: Icons.BarChart3 || Icons.Info, msg: `${type} accounts for ${rate.toFixed(1)}% of crimes` });
  });
  const suspectsForAlerts = countBy(filtered.filter((r) => r.suspectName), 'suspectName');
  Object.entries(suspectsForAlerts).filter(([, c]) => c > 1).forEach(([name, count]) => {
    alerts.push({ type: 'danger', icon: Icons.User, msg: `Repeat offender: ${name} linked to ${count} incidents` });
  });
  const locCountsForAlerts = countBy(filtered, (r) => `${r.sitio}|${r.street}`);
  Object.entries(locCountsForAlerts).filter(([, c]) => c >= 2).slice(0, 3).forEach(([loc, count]) => {
    const [sitio, location2] = loc.split('|');
    alerts.push({ type: 'warning', icon: Icons.Hotspot, msg: `Repeat location: ${location2}, ${sitio} (${count} incidents)` });
  });
  if (settings.population) {
    const crimeRate = (filtered.length / settings.population) * 1000;
    if (crimeRate > settings.threshold) {
      alerts.push({ type: 'danger', icon: Icons.Siren, msg: `Crime rate (${crimeRate.toFixed(2)}/1000) exceeds threshold (${settings.threshold})` });
    }
  }
  ...
  const regression = monthKeys.map((_, i) => +(slope * i + intercept).toFixed(1));
  ...
  const hotspots = SITIOS.map((s) => ({
    ...
    risk: (bySitioForAlerts[s] || 0) >= 5 ? 'High' : (bySitioForAlerts[s] || 0) >= 3 ? 'Medium' : 'Low',
  })).sort((a, b) => b.count - a.count);
  const repeatLocs = Object.entries(locCountsForAlerts).filter(([, c]) => c > 1).sort((a, b) => b[1] - a[1]).slice(0, 10)
    .map(([k, count]) => { const [sitio, location2] = k.split('|'); return { location: location2, sitio, count }; });
  const riskSitios = SITIOS.map((s) => ({
    sitio: s,
    count: bySitioForAlerts[s] || 0,
    rate: settings.population ? (((bySitioForAlerts[s] || 0) / (settings.population / 7)) * 1000).toFixed(2) : '—',
      ...
      <Modal open={hotspotsOpen} onClose={() => setHotspotsOpen(false)} title="Hotspots" size="lg">
        <div className="table-grid" style={{ gridTemplateColumns: '1fr' }}>
            ...
            actions={
              ...
              >
                ...
                {unreadHotspotAlertCount > 0 && (
                  <span className="notif-bell-count" style={{ position: 'static', marginLeft: 6 }}>{unreadHotspotAlertCount}</span>
                )}
              </Button>
            }
          >
            <Table
              columns={[
                ...
                { key: 'risk', label: 'Severity', render: (v) => (
                  <span style={{ color: v === 'High' ? 'var(--danger)' : v === 'Medium' ? 'var(--warning)' : 'var(--success)', fontWeight: 600 }}>{v}</span>
                ) },
              ]}
              rows={hotspots}
          ...
          <Card title="Repeat Locations" bodyClassName="table-wrap">
            <Table columns={[{ key: 'location', label: 'Location' }, { key: 'sitio', label: 'Sitio' }, { key: 'count', label: 'Incidents' }]} rows={repeatLocs} />
          </Card>
          <Card title="High-Risk Sitios" bodyClassName="table-wrap">
            <Table columns={[{ key: 'sitio', label: 'Sitio' }, { key: 'count', label: 'Incidents' }, { key: 'rate', label: 'Rate/1K' }, { key: 'level', label: 'Assessment' }]} rows={riskSitios} />
          </Card>
        </div>
      </Modal>
